 ...
 * TODO TCF 初始化加载IOC/DI依赖注入容器，基于反射给目标实例的目标属性注入属性值，基于反射调用目标实例的目标方法
 * @author 71485
 *
 */
public class IocHelper {

	//TODO TCF 初始化加载IOC/DI依赖注入容器，实现依赖注入
	static
	{
	...
	//TODO TCF 启动IOC/DI依赖注入容器，实现依赖注入
	//TODO TCF 基于反射给目标实例目标属性注入属性值，基于反射调用目标实例的目标方法
												...
												if(StringUtils.isNotEmpty(classSimpleName) && classSimpleName.equals(targetClass.getSimpleName()))
												{
													...
													if(paramMap!=null)
													{
														for(Map.Entry<String,Object> paramEntry:paramMap.entrySet())
														{
															...
															//TODO TCF 需要注入的属性名必须和当前加载的属性名相同才能注入
	...
	public static boolean injectFieldFilter(Field field)
	{
		...
		//TODO TCF 只注入使用KidInject注解标注的属性
